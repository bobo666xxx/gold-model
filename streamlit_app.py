...
def make_pdf(b):
    ...
    RED = colors.HexColor("#c0392b"); GREEN = colors.HexColor("#27ae60")
    ss = getSampleStyleSheet()
    # 版心可用宽度(左右边距各15mm): 所有表格按此宽度铺满 -> 左右完全对齐
    USABLE = A4[0] - 15 * mm - 15 * mm
    ...
    def mk(data, widths, cc=None, risk=False):
        # widths 为相对权重; 按比例归一化铺满版心宽度 USABLE -> 表格左右完全对齐
        cc = cc or []
        total = float(sum(widths)) or 1.0
        colw = [w / total * USABLE for w in widths]
        ...
        for r in data[1:]:
            ...
            rows.append(cells)
        tb = Table(rows, colWidths=colw, repeatRows=1, hAlign="LEFT")
        stl = [("BACKGROUND", (0, 0), (-1, 0), BLUE),
        ...
        for i in range(1, len(rows)):
            if i % 2 == 0:
                stl.append(("BACKGROUND", (0, i), (-1, i), LGREY))
    ...
    story.append(P("数据统计时间：%s(%s, SGE正常交易日)｜报告生成：%s（北京时间）"
                   % (M["数据截止日"], cn_week(M["数据截止日"]), b["update_time"]), st_meta))
    story.append(P("数据来源：%s" % M["数据来源"], st_meta))
    story.append(P("实时源：%s｜历史源：%s(%d日)｜美元兑人民币：%.4f%s｜国际 %.2f 美元/盎司 ≈ %.2f 元/克"
                   % (b["spot_src"], b["hist_src"], b["lookback"], b["fx"],
                      "" if b["fx_live"] else "(兜底)", b["price_usd"], b["price_cny"]), st_meta))
    story.append(Spacer(1, 3))
    story.append(P("报告导读：本速报由系统自动抓取实时行情与逐日历史序列后自动计算填全，含五大板块——"
                   "国际金价、上海金折算价、竞品零售对比、风险与影响因素、机构观点。凡海外节点无法直连的境内权威值"
                   "（上金所官方结算/延时、各品牌实时牌价、宏观数据具体值、机构最新研报）均以真实数据折算/估算并明确标注，"
                   "绝不冒充官方值。", st_note))

    story.append(sect("核心数据速览"))
    t0 = [["核心指标", "数值", "口径 / 来源说明"],
          ["国际金价现货", "%.2f 美元/盎司" % b["price_usd"], "实时源 %s" % b["spot_src"]],
          ["伦敦金较上一日", "%s" % chg_txt, "基于逐日历史序列自动计算"],
          ["国内折算参考价", "%.2f 元/克" % b["price_cny"], "现价×在岸汇率÷31.1035"],
          ["美元兑人民币", "%.4f" % b["fx"], "实时(er-api/frankfurter)" + ("" if b["fx_live"] else "(兜底)")],
          ["白银现货", ("%.2f 美元/盎司" % b["xag"]) if b["xag"] else "—", "实时源 %s" % b["spot_src"]],
          ["SGE Au99.99 折算收盘", "%s 元/克" % au[4], "折算参考(非官方结算)"],
          ["综合风险等级", X["风险等级"], "按单日涨跌幅与近期振幅自动判定"]]
    story.append(mk(t0, [120, 150, 240], cc=[1]))

    story.append(sect("一、国际金价"))
    story.append(mk(T["t1"], [120, 100, 66, 150], cc=[1, 2]))
    story.append(Spacer(1, 3))
    story.append(P(X["关键事件"]))
    story.append(P("联动提示：金价以美元计价，与美元指数、实际利率呈反向关系，与避险情绪、央行购金呈正向关系；"
                   "本板块价格为海外直连实时值，COMEX 期货按常规升水估算，换算人民币口径为国际价×在岸汇率÷31.1035。", st_note))

    story.append(sect("二、上海黄金交易所价格（折算参考口径）"))
    story.append(P("▎日盘数据（%s 9:00-15:30，折算参考价）" % M["数据截止日"], st_sub))
    story.append(mk(T["t2"], [78, 64, 64, 64, 64, 70, 78], cc=list(range(1, 7))))
    story.append(Spacer(1, 2))
    story.append(P(X["重点关注"]))
    story.append(P("▎夜盘数据（归属下一交易日清算，折算参考价）", st_sub))
    story.append(mk(T["t3"], [78, 100, 92, 92, 150], cc=[1, 2, 3, 4]))
    story.append(Spacer(1, 2))
    story.append(P(X["夜盘解读"]))
    story.append(P(X["注1"], st_note))
    story.append(P(X["注2"], st_note))
    story.append(P("说明：上金所成交量为双向计量、持仓数据以官方延时行情为准；本页折算价仅反映国际金价在国内的等值水平，"
                   "实际撮合价受境内供需、汇率与交割升贴水影响，可能与折算值存在小幅偏差。", st_note))

    story.append(sect("三、竞品零售报价对比（估算口径）"))
    story.append(P("▎足金饰品（元/克，原料价+常见工费/品牌溢价估算）", st_sub))
    story.append(mk(T["t3a"], [96, 78, 78, 200], cc=[1, 2]))
    story.append(P("▎投资金条（元/克，原料价+常见升水估算）", st_sub))
    story.append(mk(T["t3b"], [170, 90, 90], cc=[1, 2]))
    story.append(Spacer(1, 2))
    story.append(P(X["价差"]))
    story.append(P("采购提示：水贝批发价与原料价差距最小、最贴近原料成本，适合批量采购；品牌零售含较高工费与品牌溢价，"
                   "投资金条升水显著低于首饰金，配置性采购可优先考虑银行/品牌投资金条或交易所原料。", st_note))

    story.append(sect("四、风险提示与金价影响因素分析"))
    story.append(P("▎风险等级", st_sub))
    story.append(mk([["风险等级", "内容"]] + T["t4"], [80, 440], cc=[0], risk=True))
    for title, items in X["宏观"].items():
        story.append(P("▶ %s" % title, st_sub))
        for it in items:
            story.append(P("· " + it))
    story.append(P("▎技术面关键位（历史区间与均线推导）", st_sub))
    story.append(mk([["类型", "国际金价(美元/盎司)", "国内SGE(元/克)"]] + T["t5"],
                    [150, 180, 190], cc=[1, 2]))
    story.append(Spacer(1, 2))
    story.append(P(X["短期预判"]))
    story.append(P(X["技术注"], st_note))

    story.append(sect("五、机构观点"))
    story.append(mk(T["t6"], [120, 70, 130, 200], cc=[1]))
    story.append(Spacer(1, 3))
    story.append(P(X["共识"]))
    story.append(Spacer(1, 3))
    story.append(P("数据与方法说明：实时金价来自 gold-api.com（失败回落新浪 hf_XAU），汇率来自 er-api/frankfurter，"
                   "历史序列来自 Yahoo GC=F（失败回落波兰央行 NBP 折算）；涨跌幅、振幅、支撑阻力、均线、价差与风险等级"
                   "均由上述真实数据自动推导。品牌零售、投资金条、宏观具体数值与机构目标价为口径标注的估算/待核实项，"
                   "使用前请以对应官方终端复核。", st_note))
